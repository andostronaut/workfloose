...
const generate = ({ answers }) => {
  ...
  spinner.start(`Scaffold ${environemt} CI/CD`)

  fs.cp(src, dest, { recursive: true }, async (err) => {
    if (err) {
      return log(
        chalk.red(`An error occured while scaffolding ${environemt} CI/CD.`, err),
        constants.LOG_TYPE_ERROR
      )
    }
    ...
    await read(dest)
      .then(() => {
        ...
        if (version) {
          yml.image = `${environemt}${
            environemt === constants.GO_ENVIRONMENT ? 'lang' : ''
          }:${customVersion}`
        }
    ...
    spinner.succeed(`${_.capitalize(environemt)} CI/CD scaffolded!`)
  })
}
