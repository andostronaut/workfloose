...
const generate = ({ answers }) => {
  ...
  if (workflows.includes(constants.CD_WORKFLOW)) {
    const src = path.join(__dirname, `${constants.TEMPLATE_PATH}/${platform}/cd/${deployer}-cd.yml`)
    ...
    if (generateProcfile) {
      spinner.start('Generate Procfile')

      execa('touch', ['Procfile']).then(() => {
        const dest = path.join(process.cwd(), 'Procfile')
        const content = constants.PROCFILE_CONFIG[environemt]

        fs.writeFile(dest, content, (err) => {
          if (err) {
            return log(
              chalk.red('An error occured while generating Procfile', err),
              constants.LOG_TYPE_ERROR
            )
          }

          spinner.succeed('Procfile generated')
        })
      })
    }
    ...
    copy({
      src,
      dest,
      spinnerMsg: `Scaffold ${environemt} CD`,
      errorMsg: `An error occured while scaffolding ${environemt} CD.`,
      successMsg: `${_.capitalize(environemt)} CD scaffolded!`
    })
  }
